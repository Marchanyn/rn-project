...
getMonthTasks = (tasks, currentDate) => {
  return tasks.filter(task => {
    const taskDate = new Date(task.timestamp);
...
getYearTasks = (tasks, currentDate) => {
  return tasks.filter(task => {
    const taskDate = new Date(task.timestamp);
...
const Expenenss = () => {
      ...
      case 'Day':
        selectedDate =
          ...
          new Date(timeOfDay)
            .toLocaleDateString('Ru', {month: 'long'})
            .slice(1);
        return getDayTasks(tasks, currentDate);
      case 'Week':
        selectedDate =
          ...
          new Date(timeOfWeek + 1000 * 60 * 60 * 24 * 7)
            .toLocaleDateString('Ru', {month: 'long'})
            .slice(1, 3);
        return getWeekTasks(tasks, currentDate);
      case 'Month':
        selectedDate =
        ...
        return getMonthTasks(tasks, currentDate);
      case 'Year':
        selectedDate = new Date(timeOfYear).getFullYear();
        return getYearTasks(tasks, currentDate);
      default:
        selectedDate = '';
        return tasks;
    }
  ...
  const getPervPeriod = () => {
    switch (selectedPeriod) {
      case 'Day':
        return switchOnPervDay();
      case 'Week':
        return switchOnPervWeek();
      case 'Month':
        return switchOnPervMonth();
      case 'Year':
        return switchOnPervYear();
    }
  };
  const getNextPeriod = () => {
    switch (selectedPeriod) {
      case 'Day':
        return switchOnNextDay();
      case 'Week':
        return switchOnNextWeek();
      case 'Month':
        return switchOnNextMonth();
      case 'Year':
        return switchOnNextYear();
    }
  };
  ...
  const totalExpenses = Object.values(chartData).reduce(
    (acc, curr) => acc + curr,
    0,
  );
